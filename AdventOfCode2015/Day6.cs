...
turn on 222,12 through 856,241";
            ...
            Dictionary<Vector2, int> lights = new Dictionary<Vector2, int>();

            for (int x = 0; x < 1000; x++) {
                for (int y = 0; y < 1000; y++) {
                    lights.Add(new Vector2(x, y), 0);
                }    
            }
            ...
            foreach (string line in input.Split("\n")) {
                ...
                if (line.StartsWith("turn on")) {
                    ...
                    string firstCoords = l.Split(" ")[1];
                    string secondCoords = l.Split(" ")[3];
                    
                    Vector2 first = new Vector2(float.Parse(firstCoords.Split(",")[0]), float.Parse(firstCoords.Split(",")[1]));
                    Vector2 second = new Vector2(float.Parse(secondCoords.Split(",")[0]), float.Parse(secondCoords.Split(",")[1]));

                    List<Vector2> between = GetInBetween(first, second);

                    foreach (Vector2 btwn in between) {
                        lights[btwn] += 1;
                    }
                }
                ...
                if (line.StartsWith("turn off")) {
                    ...
                    foreach (Vector2 btwn in between) {
                        if (lights[btwn] != 0) lights[btwn] -= 1;
                    }
                }
                ...
                Console.WriteLine("There is {0} total brightness levels!", total);
                
            }
        }

        static List<Vector2> GetInBetween(Vector2 first, Vector2 second) {
            List<Vector2> output = new List<Vector2>();

            for (float x = Math.Min(first.X, second.X); x <= Math.Max(first.X, second.X); x++) {
                for (float y = Math.Min(first.Y, second.Y); y <= Math.Max(first.Y, second.Y); y++) {
